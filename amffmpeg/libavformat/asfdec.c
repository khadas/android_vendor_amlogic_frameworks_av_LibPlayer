...
static void print_guid(const ff_asf_guid *g)
{
    ...
    PRINT_IF_GUID(g, ff_asf_header);
        ...
        PRINT_IF_GUID(g, ff_asf_audio_conceal_none);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_video_stream);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_video_conceal_none);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_command_stream);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_comment_header);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_codec_comment_header);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_codec_comment1_header);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_data_header);
    }
    else {
        PRINT_IF_GUID(g, index_guid);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_head1_guid);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_head2_guid);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_my_guid);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_ext_stream_header);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_extended_content_header);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_ext_stream_embed_stream_header);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_ext_stream_audio_stream);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_metadata_header);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_marker_header);
    }
    else {
        PRINT_IF_GUID(g, stream_bitrate_guid);
    }
    else {
        PRINT_IF_GUID(g, ff_asf_language_guid);
    }
    else {
        ...
        if (!ff_guidcmp(&g, &ff_asf_file_header)) {
            ...
            if (!s->keylen) {
                if (!ff_guidcmp(&g, &ff_asf_content_encryption)) {
                    ...
                    av_hex_dump_log(s, AV_LOG_WARNING, pkt.data, pkt.size);
                    av_free_packet(&pkt);
                    len = avio_rl32(pb);
