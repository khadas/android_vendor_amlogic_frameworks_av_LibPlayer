...
static int mp4_get_codec_tag(AVFormatContext *s, MOVTrack *track)
{
    ...
    if (track->enc->codec_id == CODEC_ID_H264) {
        ...
        tag = MKTAG('a', 'c', '-', '3');
    } else if (track->enc->codec_id == CODEC_ID_DIRAC) {
        tag = MKTAG('d', 'r', 'a', 'c');
    } else if (track->enc->codec_id == CODEC_ID_MOV_TEXT) {
        tag = MKTAG('t', 'x', '3', 'g');
...
static int mov_get_dv_codec_tag(AVFormatContext *s, MOVTrack *track)
{
    ...
    if (track->enc->width == 720) /* SD */
        ...
        else if (track->enc->pix_fmt == PIX_FMT_YUV422P) {
            tag = MKTAG('d', 'v', '5', 'p');
        ...
        if (track->enc->time_base.den == 50) {
            tag = MKTAG('d', 'v', 'h', 'q');
        } else {
            tag = MKTAG('d', 'v', 'h', 'p');
        }
    else if (track->enc->height == 1080) /* HD 1080 line */
        if (track->enc->time_base.den == 25) {
            tag = MKTAG('d', 'v', 'h', '5');
        } else {
            tag = MKTAG('d', 'v', 'h', '6');
        }
...
static int mov_write_header(AVFormatContext *s)
{
    ...
    if (s->oformat != NULL) {
        if (!strcmp("3gp", s->oformat->name)) {
            mov->mode = MODE_3GP;
        } else if (!strcmp("3g2", s->oformat->name)) {
            mov->mode = MODE_3GP | MODE_3G2;
        } else if (!strcmp("mov", s->oformat->name)) {
            mov->mode = MODE_MOV;
        } else if (!strcmp("psp", s->oformat->name)) {
            mov->mode = MODE_PSP;
        } else if (!strcmp("ipod", s->oformat->name)) {
            mov->mode = MODE_IPOD;
        }
...
static int mov_write_trailer(AVFormatContext *s)
{
    ...
    for (i = 0; i < mov->nb_streams; i++) {
        ...
        if (mov->tracks[i].vosLen) {
            av_free(mov->tracks[i].vosData);
        }
